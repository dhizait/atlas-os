<#
.SYNOPSIS
PS98-ATLAS-BackupRestore.ps1 - Backup and Restore for ATLAS R17 v1.0
#>
...
function Write-Log {
    ...
    Write-Host "[$time] [$Level] PS98-ATLAS: $Msg" -ForegroundColor $color
}
...
function Backup-Project {
    ...
    $excludeDirs = "node_modules", ".git", "dist", "build", ".next", "target", "__pycache__", ".venv", "docker_data"
...
function Restore-Project {
    ...
    $ProjectBackupDir = Join-Path $BackupRoot $ProjectName
    $LatestBackup = Get-ChildItem "$ProjectBackupDir\backup-*" -Directory -ErrorAction SilentlyContinue |
    ...
    $excludeDirs = "node_modules", ".venv", ".git", "dist", "build", ".next", "target", "docker_data"
    $excludeArgs = $excludeDirs | ForEach-Object { "/XD $_" }
...
switch ($Action) {
    "Backup" {
        Backup-Project -Source $ProjectPath -ProjectName $ProjectName
        exit 0
    }
    "Restore" {
        Restore-Project -Dest $ProjectPath -ProjectName $ProjectName
        exit 0
    }
    "Test" {
        Write-Log "Test mode active" "INFO"
        exit 0
    }
}